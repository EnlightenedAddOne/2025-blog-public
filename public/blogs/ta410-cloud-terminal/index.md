...
- **锁定情况**：开机即进入"**洛阳理工学院**"校园网的远程桌面连接器（Kiosk 全屏锁定界面），需学号密码登录，且写死了内网虚拟桌面服务器地址。离开校园网即"无法连接服务器"，普通用户无法进入任何本地系统。
...
| # | 问题 | 状态 | 说明 |
|---|---|---|---|
| 1 | 原厂系统锁定（洛阳理工学院） | ✅ 已解决 | TF 卡刷入 Armbian 绕过 |
...
9. 手机 SSH 连板子（`root@192.168.70.3`，密码 `1234`）。
...
4. 或通过 USB 网络共享 SSH：`root@192.168.70.3`，密码 `1234`。
...
- **原厂系统**：洛阳理工学院定制的 Thinclient OS，锁定在校园网，无法本地使用。
